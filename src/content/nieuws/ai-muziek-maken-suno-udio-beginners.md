---
title: "AI-muziek maken met Suno en Udio: van tekstprompt naar nummer"
heroImageAlt: "Miniatuur diorama-illustratie bij artikel 'AI-muziek maken met Suno en Udio: van tekstprompt naar nummer'"
description: "Met Suno en Udio genereer je originele muziek met een tekstprompt — van achtergrondmuziek tot complete nummers met vocalen. Dit is hoe je begint, welke stijlen werken en wanneer je welke tool kiest."
publishedAt: 2026-06-09
updatedAt: 2026-06-09
author: "Redactie"
category: "gids"
tags:
  - "suno"
  - "udio"
  - "ai-muziek"
  - "muziekgeneratie"
  - "audio"
  - "creatief"
toolSlug: "suno"
featured: false
draft: false
readingTime: 4
heroImage: "/images/articles/diorama-ai-muziek-maken-suno-udio-beginners.webp"
heroScene: "A tiny wooden stage with miniature musical instruments — a small guitar, piano keys, and floating music notes — glowing softly in studio lighting, with a translucent waveform above"
keyTakeaways:
  - "Suno v4 is de standaard voor complete nummers met vocalen — je beschrijft een stijl en de AI schrijft én zingt het nummer."
  - "Udio blinkt uit in instrumentale muziek en geeft meer controle over het geluid en de structuur."
  - "Beide tools hebben gratis plans met een beperkt aantal nummers per maand — genoeg om te testen."
  - "Commercieel gebruik vereist een betaald plan bij beide tools; check de licentievoorwaarden voor jouw gebruik."
  - "De promptformule 'stijl + instrument + tempo + sfeer' geeft consistent betere resultaten dan een vage beschrijving."
faq:
  - q: "Mag je AI-muziek van Suno of Udio gebruiken in een YouTube-video?"
    a: "Op het gratis plan van Suno en Udio is gebruik beperkt tot niet-commercieel. Voor gebruik in YouTube-video's met monetisatie heb je een betaald plan nodig (Suno Pro: $8/maand, Udio Standard: $10/maand). Controleer altijd de actuele licentievoorwaarden van de tool — die worden geregeld bijgewerkt."
  - q: "Kan Suno ook Nederlandse liedjes maken?"
    a: "Ja. Voeg in je prompt toe 'lyrics in Dutch' of schrijf de songtekst zelf in het Nederlands in het 'Custom Mode' en laat Suno die zingen. De uitspraak is redelijk, maar voor perfecte uitspraak van dialectwoorden of complexe klanken is Engels nog wat sterker."
  - q: "Hoe lang kan een AI-nummer zijn?"
    a: "Suno genereert standaard nummers van circa 2-4 minuten. Met de 'Extend'-functie kun je een nummer verlengen door er nieuwe secties aan toe te voegen. Udio werkt vergelijkbaar. Nummers van 5+ minuten zijn mogelijk maar vereisen wat meer handmatig samenvoegen."
  - q: "Kan ik mijn eigen melodie of akkoordenschema als input gebruiken?"
    a: "Suno en Udio werken primair met tekst-prompts. Voor meer muzikale controle — zoals een eigen MIDI-bestand of een specifiek akkoordenschema — zijn tools als Stable Audio of Udio's geavanceerde modus beter geschikt."
  - q: "Zijn er alternatieven voor Suno en Udio?"
    a: "Ja: Mubert is goed voor royalty-vrije achtergrondmuziek op basis van stijl en stemming. ElevenLabs heeft ook muziek-functies. Stable Audio van Stability AI is open-source vriendelijker en geeft meer technische controle."
---

Een jingle voor een podcast, achtergrondmuziek voor een video of gewoon curiositeit — AI-muziek is inmiddels goed genoeg dat je het écht kunt gebruiken. Geen muzikale kennis nodig. Dit is hoe je begint.

## Suno vs. Udio: wanneer gebruik je welke?

| | **Suno v4** | **Udio** |
|---|---|---|
| Beste voor | Complete nummers met vocalen | Instrumentale tracks, meer controle |
| Gratis plan | 50 credits/dag (~10 nummers) | 10 tracks/dag |
| Betaald | $8/maand (Pro, 2.500 credits) | $10/maand (Standard, 1.200 credits) |
| Stijl-bereik | Zeer breed (pop tot metal tot klassiek) | Breed, sterk in jazz, elektronisch |
| Custom tekst | Ja (Custom Mode) | Ja |
| Commercieel gebruik | Betaald plan vereist | Betaald plan vereist |

> **💡 Beginner-tip:** Ga naar [suno.com](https://suno.com) en maak een gratis account. Je hebt geen creditcard nodig. Typ een eerste simpele prompt en druk op Create — binnen 30 seconden luister je naar een nummer dat de AI helemaal zelf heeft gemaakt. Daarna ga je verfijnen.

## De promptformule die werkt

Vage prompts geven generieke muziek. Gebruik deze structuur:

**[Genre] + [instrumenten] + [tempo] + [sfeer] + [optioneel: vocalen of instrumentaal]**

❌ `vrolijk liedje voor een webshop`

✓ `upbeat acoustic pop, ukulele and light percussion, 120 BPM, cheerful and summery, female vocals, suitable for a lifestyle brand video`

✓ `melancholic lo-fi hip hop, muted piano and vinyl crackle, slow tempo, rainy afternoon feeling, instrumental`

Engelstalige prompts geven bij beide tools iets betere resultaten dan Nederlandse, maar het verschil wordt kleiner.

## Stap 1: maak je eerste nummer in Suno

1. Ga naar [suno.com](https://suno.com) en log in
2. Klik op **Create**
3. Schakel naar **Custom Mode** (rechtsboven in het invoerscherm) voor meer controle
4. Vul in:
   - **Style of Music**: jouw promptformule (zie hierboven)
   - **Lyrics**: schrijf de tekst zelf, of laat hem leeg en vul in `[Instrumental]`
   - **Title**: geeft de AI extra context over de sfeer
5. Klik op **Create** — Suno genereert twee versies tegelijk

Luister naar beide versies. Eén is bijna altijd beter dan de ander. Klik op **Reuse Prompt** als je met dezelfde stijl nieuwe varianten wilt genereren.

## Stap 2: verleng of remix een nummer

Als je een intro of vers hebt dat je mooi vindt maar het nummer te kort is: klik op de drie puntjes bij het nummer → **Extend**. Suno genereert een naadloze verlenging in dezelfde stijl.

Wil je een bestaand nummer in een andere stijl? Gebruik **Cover** of **Remix** om de melodie te behouden maar de stijl te veranderen — van akoestisch naar elektronisch, van Engels naar Nederlands.

> **⚡ Gevorderd:** In Suno's Custom Mode kun je liedteksten structureren met secties: `[Verse]`, `[Chorus]`, `[Bridge]`, `[Outro]`. De AI past de muziekstructuur aan op jouw aanduidingen. Zo bouw je een nummer dat precies de opbouw heeft die je wilt, in plaats van te accepteren wat de AI kiest.

## Stap 3: instrumentale achtergrondmuziek in Udio

Voor video-achtergronden, podcasts of presentaties is Udio vaak de betere keuze omdat je meer controle hebt over het geluid zonder vocalen.

1. Ga naar [udio.com](https://udio.com)
2. Typ je prompt (stijl, instrumenten, sfeer)
3. Voeg `no vocals`, `instrumental` toe aan je prompt
4. Genereer en kies de beste versie

Udio laat je ook kleine aanpassingen doen aan de audio nadat het is gegenereerd — een handige extra stap om het nummer beter te laten aansluiten bij jouw video.

## Checklist: ben je klaar?

- [ ] Gratis account aangemaakt bij Suno of Udio
- [ ] Prompt gebruikt met: genre + instrumenten + tempo + sfeer
- [ ] Twee gegenereerde versies vergeleken en de beste gekozen
- [ ] Voor commercieel gebruik: betaald plan geactiveerd en licentievoorwaarden gelezen
- [ ] Nummer gedownload in het gewenste formaat (MP3/WAV)

## Bronnen

- [Suno — Terms of Service en licentie-informatie](https://suno.com/terms)
- [Udio — Pricing en licenties](https://www.udio.com/pricing)
- [The Verge — AI music generators 2026 review](https://www.theverge.com)
