...
Drie dingen die je in de eerste week tegenkomt. Eén: het cap van 1000 subagents is hard, maar bij langere runs vraagt Claude soms tussentijds bevestiging om door te gaan — pas je workflow in op kleinere batches als je dat patroon ziet. Twee: dynamic workflows draait in research preview, wat betekent dat Anthropic exacte limieten, naamgeving van slash-commando's en defaults zonder veel waarschuwing kan veranderen. Bouw geen productie-pipeline op een exact aantal subagents. Drie: de kosten lopen sneller op dan een normale Opus-sessie omdat elke subagent een eigen call doet. Begin altijd met een kleine test-repo of een afgebakende sub-taak voor je iets codebase-breed afvuurt. Nog niet vertrouwd met de basis-lus achter dit alles? Zet dan eerst [je eerste agent-loop met Claude Code](/nieuws/je-eerste-agent-loop-claude-code) op voor je opschaalt naar parallelle agents. Wil je de tokens-teller sowieso temmen, dan kan [de Caveman-skill je output tot 65% inkorten](/nieuws/caveman-claude-code-tokens-besparen). Nog niet thuis in Claude Code zelf? De [gratis Claude Code-cursus op Anthropic Academy](/nieuws/anthropic-academy-gratis-ai-cursussen) loopt de basis met je door. En wil je dit soort agents in productie draaien zonder eigen infrastructuur, lees dan onze gids over [Claude Managed Agents](/nieuws/claude-managed-agents-bouwen). Let bij het uitbreiden van je opstelling op de contextkosten: het [plugin-detailscherm toont vooraf hoeveel tokens een bundel per beurt kost](/nieuws/claude-code-plugins-installeren).

Eerlijk: onze eerste dynamic-workflow-poging was een 50-file refactor met de prompt "fix alle deprecated APIs". Claude koos zelf voor 80 subagents, de run kostte 12 minuten en het resultaat was 90% correct — maar die laatste 10% bevatte één breaking change die we pas door de tests opvingen. Les: behandel een workflow-output als een grote pull request, niet als een gegarandeerd correct resultaat. Test draaien en review hoort erbij. Voor wie naast Claude Code ook Cursor of GitHub Copilot gebruikt, blijft onze [Claude-vs-ChatGPT-vergelijking](https://www.aiplatformmkb.nl/tools/chatgpt-claude-gemini-copilot-welke-past-bij-jou) een handige context-check voor model-keuze per taak. Werk je in een onbekende codebase voordat je zo'n grote workflow loslaat? Dan helpt [Understand-Anything om je codebase eerst als kennisgraaf in kaart te brengen](/nieuws/understand-anything-codebase-kennisgraaf). En wil je meerdere van die workflows tegelijk volgen, dan laat de [herontworpen desktop-app meerdere agents parallel draaien](/nieuws/claude-code-desktop-parallelle-sessies), elk in een eigen git-worktree.

Let op wat je daarmee níet kunt terugdraaien: [rewind herstelt geen subagent-edits](/nieuws/claude-code-rewind-checkpoints). Werk je liever in VS Code met GitHub in plaats van Claude Code, dan is [Copilot's agent mode](/nieuws/copilot-agent-mode-vscode) het vergelijkbare startpunt, zij het zonder de dynamic-workflows-schaal van honderden subagents.
