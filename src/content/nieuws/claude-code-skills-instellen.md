...
Skills zijn één van de bouwstenen waarmee Claude Code de afgelopen tijd is uitgegroeid tot een compleet platform, naast plugins, hooks en de parallelle [Dynamic Workflows](/nieuws/claude-code-dynamic-workflows-gebruiken) voor grotere klussen. Sinds het herontwerp van april 2026 draai je die skills bovendien in een desktop-app met meerdere sessies naast elkaar — zie onze gids over [werken met parallelle agents in de Claude Code desktop-app](/nieuws/claude-code-desktop-parallelle-sessies), of buiten de desktop-app om met [Solo als agent-werkplek](/nieuws/solo-terminal-agents-een-venster). Voor terugkerend werk is een eigen skill vaak het simpelste begin — en wil je de agent zichzelf laten aansturen in plaats van elke stap te typen, dan bouw je met dezelfde `CLAUDE.md` en een hook [je eerste agent-loop](/nieuws/je-eerste-agent-loop-claude-code). Een concreet test-recept dat zich goed voor zo'n skill leent: [je signup-flow inclusief e-mailverificatie laten testen met Playwright](/nieuws/claude-code-signupflow-testen-playwright). Benieuwd wat zo'n agentic sessie in de praktijk oplevert? In [onze workflow met Higgsfield bouwt Claude Code een complete geanimeerde website](/nieuws/claude-code-higgsfield-geanimeerde-website).  Een populair kant-en-klaar voorbeeld is [Caveman, dat tot 65% tokens bespaart](/nieuws/caveman-claude-code-tokens-besparen) door Claude korter te laten antwoorden. Wil je Claude Code eerst van de grond af leren? Anthropic heeft er een [gratis cursus met certificaat](/nieuws/anthropic-academy-gratis-ai-cursussen) voor.
