...
Voor de bredere context over waarom en wanneer browser-inferentie het juiste architectuurpatroon is — inclusief de beperkingen qua modelgrootte — lees de analyse op Het Laatste AI Nieuws: [AI in de browser zonder server](https://www.hetlaatsteainieuws.nl/achtergrond/ai-in-de-browser-zonder-server). ([Bron: Hugging Face](https://huggingface.co/docs/transformers.js/index)) En vraag je je af wat "transformer" in de naam eigenlijk betekent? Ons mentale model van [CNN's, RNN's en transformers](/nieuws/cnn-rnn-transformers-huggingface-uitleg) legt het zonder wiskunde uit.
